        ...
        <div>
            <div className="w-full max-w-md mx-auto bg-background p-4 border-2 border-foreground font-sans">
                ...
                <div className="text-sm">
                    ...
                    <div className='w-full flex flex-row items-center justify-between gap-1 pt-4 mt-4 border-t border-border'>
                        <Logo className='h-6 flex-none' />
                        <span className='text-xs text-muted-foreground'>Visual Food Intelligence</span>
                    </div>
                </div>
            </div>
        </div>
...
const NutritionItem: React.FC<NutritionItemProps> = ({ name, value, unit, indented = false }) => {
    ...
    return (
        <div className={`flex justify-between ${indented ? 'pl-4' : ''} border-t border-foreground`}>
            <span><span>{name}</span> {value}{unit}</span>
